...
rm -rf SHWireGuardKit.xcframework

# remove the old copy of the xcframework zip if it already exists
rm -rf SHWireGuardKit.xcframework.zip

xcodebuild -sdk iphonesimulator -target SHWireGuardKitiOS
xcodebuild -sdk iphoneos -target SHWireGuardKitiOS
xcodebuild -sdk macosx -target SHWireGuardKitmacOS
...
xcodebuild -create-xcframework -framework "$sim_fwpath" -output "$name".xcframework
...
ditto -c -k --sequesterRsrc --keepParent "./SHWireGuardKit.xcframework" "./SHWireGuardKit.xcframework.zip"

printf "\n"
printf "SwiftPM .zip checksum:\n"
# get hash checksum for SwiftPM
swift package compute-checksum "./SHWireGuardKit.xcframework.zip"
