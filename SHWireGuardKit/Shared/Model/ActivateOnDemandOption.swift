...
// MARK: - ActivateOnDemandSSIDOption
...
extension ActivateOnDemandOption {
  func apply(on tunnelProviderManager: NETunnelProviderManager) {
    ...
    case .off:
      rules = nil
    case let .wiFiInterfaceOnly(ssidOption):
      rules = ssidOnDemandRules(option: ssidOption) +
    ...
    case let .anyInterface(ssidOption):
  ...
  init(from tunnelProviderManager: NETunnelProviderManager) {
    if tunnelProviderManager.isOnDemandEnabled,
       let onDemandRules = tunnelProviderManager.onDemandRules {
  ...
  private static func create(from rules: [NEOnDemandRule]) -> ActivateOnDemandOption {
    ...
    case 2:
      guard let connectRule = rules.first(where: { $0.action == .connect }) else {
        wg_log(
          .error,
          message: "Unexpected onDemandRules set on tunnel provider manager: \(rules.count) rules found but no connect rule."
        )
        return .off
      }
      guard let disconnectRule = rules.first(where: { $0.action == .disconnect }) else {
        wg_log(
          .error,
          message: "Unexpected onDemandRules set on tunnel provider manager: \(rules.count) rules found but no disconnect rule."
        )
        return .off
      }
      if connectRule.interfaceTypeMatch == .wiFi,
        ...
        wg_log(
          .error,
          message: "Unexpected onDemandRules set on tunnel provider manager: \(rules.count) rules found but interface types are inconsistent."
        )
        return .off
      }
    case 3:
      guard let ssidRule = rules
        .first(where: { $0.interfaceTypeMatch == .wiFi && $0.ssidMatch != nil })
      else { return .off }
      guard let nonWiFiRule = rules
        .first(where: { $0.interfaceTypeMatch == nonWiFiInterfaceType }) else { return .off }
      ...
      default:
        wg_log(
          .error,
          message: "Unexpected onDemandRules set on tunnel provider manager: \(rules.count) rules found"
        )
        return .off
      }
    default:
      wg_log(
        .error,
        message: "Unexpected number of onDemandRules set on tunnel provider manager: \(rules.count) rules found"
      )
      return .off
    }
  }
...
extension NEOnDemandRuleConnect {
  fileprivate convenience init(
    interfaceType: NEOnDemandRuleInterfaceType,
    ssids: [String]? = nil
  ) {
    self.init()
...
extension NEOnDemandRuleDisconnect {
  fileprivate convenience init(
    interfaceType: NEOnDemandRuleInterfaceType,
    ssids: [String]? = nil
  ) {
